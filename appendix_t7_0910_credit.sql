...
-- 名单：wangchuanliang.t7recalllist_0910
-- 触达日：2026-09-10
...
SELECT DISTINCT churn_user_id::bigint AS user_id,
       ...
       churn_days
FROM wangchuanliang.t7recalllist_0910
...
-- ---------------------------------------------------------------------------
WITH params AS (
    SELECT DATE '2026-09-10' AS recall_dt,
...
WITH pass_u AS (
  ...
  FROM tmp_t7_u u
  INNER JOIN wangchuanliang.order_vir_f_copy v ON v.user_id = u.user_id
  WHERE v.vir_date >= DATE '2026-09-10' AND v.vir_date <= (CURRENT_DATE - 1) AND v.is_pass1 = 1
  GROUP BY v.user_id
),
fa AS (
  ...
  INNER JOIN order_loan_f_v2_copy o
   ...
   AND o.apply_date >= DATE '2026-09-10'
...
FROM tmp_t7_u u
INNER JOIN order_loan_f_v2_copy o ON o.user_id = u.user_id
WHERE o.apply_date >= DATE '2026-09-10'
